...
class PushSubscription(db.Model):
    ...
    auth_key = db.Column(db.String(200))     # Auth secret
    ...
    failed_count = db.Column(db.Integer, default=0)  # Track delivery failures
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'endpoint'),
    )
...
class NotificationPreference(db.Model):
    ...
    push_replies = db.Column(db.Boolean, default=True)
    push_ama_reminders = db.Column(db.Boolean, default=True)
    push_deal_alerts = db.Column(db.Boolean, default=True)
    push_mentorship = db.Column(db.Boolean, default=True)
    push_system = db.Column(db.Boolean, default=True)
    
    # Notification types - Email
    email_mentions = db.Column(db.Boolean, default=True)
    email_likes = db.Column(db.Boolean, default=False)  # Off by default (too many)
    email_comments = db.Column(db.Boolean, default=True)
    email_follows = db.Column(db.Boolean, default=True)
    email_replies = db.Column(db.Boolean, default=True)
    email_ama_reminders = db.Column(db.Boolean, default=True)
    email_deal_alerts = db.Column(db.Boolean, default=True)
...
# =============================================================================
# POST SCORES (Pre-calculated for feed algorithm)
# =============================================================================
...
class PostScore(db.Model):
    ...
    score = db.Column(db.Float, default=0.0, index=True)  # Final combined score
    engagement_score = db.Column(db.Float, default=0.0)
    quality_score = db.Column(db.Float, default=1.0)
    decay_score = db.Column(db.Float, default=1.0)
    
    # Trending metrics (velocity of engagement)
    engagement_velocity = db.Column(db.Float, default=0.0)  # Engagement gained per hour
...
class UserInterest(db.Model):
    ...
    # Interest type: 'hashtag', 'room', 'specialty', 'author'
    interest_type = db.Column(db.String(20), nullable=False)
    
    # Reference ID (hashtag_id, room_id, specialty string, user_id)
    reference_id = db.Column(db.String(100), nullable=False)
    
    # Affinity score (how much user likes this)
...
class UserFeedPreference(db.Model):
    ...
    # Feed style preference
    feed_style = db.Column(db.String(20), default='algorithmic')  # 'algorithmic', 'chronological', 'following'
    
    # Content preferences
    show_anonymous = db.Column(db.Boolean, default=True)
    preferred_content_types = db.Column(db.JSON, default=list)  # ['text', 'image', 'video']
    
    # Specialty filter (show posts from these specialties)
...
# =============================================================================
# ENGAGEMENT HISTORY (For velocity calculation)
# =============================================================================
...
class Notification(db.Model):
    ...
    # The user/entity that triggered the notification (optional)
    actor_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    
    # Reference to related content
    post_id = db.Column(db.Integer, db.ForeignKey('posts.id'))
    comment_id = db.Column(db.Integer, db.ForeignKey('comments.id'))
    ama_id = db.Column(db.Integer, db.ForeignKey('expert_amas.id'))
    deal_id = db.Column(db.Integer, db.ForeignKey('investment_deals.id'))
    
    # Notification content
    title = db.Column(db.String(200))
    message = db.Column(db.Text)
    url = db.Column(db.String(500))  # Link to relevant page
...
class PostHashtag(db.Model):
    ...
    post = db.relationship('Post', backref=db.backref('hashtags', lazy='dynamic'))
    hashtag = db.relationship('Hashtag', backref=db.backref('posts', lazy='dynamic'))
    
    __table_args__ = (db.UniqueConstraint('post_id', 'hashtag_id'),)
...
class Mention(db.Model):
    ...
    mentioning_user = db.relationship('User', foreign_keys=[mentioning_user_id], backref='mentions_made')
    post = db.relationship('Post', backref='mentions')
    comment = db.relationship('Comment', backref='mentions')


# =============================================================================
# USER FOLLOWS (for @mention suggestions and notifications)
# =============================================================================
