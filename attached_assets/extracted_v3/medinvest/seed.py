...
def seed_database():
    ...
    with app.app_context():
        ...
        # =================================================================
        # POST SCORES (Initialize algorithm)
        # =================================================================
        print("  Calculating initial post scores...")
        
        from models import PostScore, Hashtag, PostHashtag
        import math
        
        # Create some sample hashtags
        hashtag_names = ['BackdoorRoth', 'RealEstate', 'IndexFunds', 'PSLF', 'FIRE', 
                        'Retirement', 'TaxStrategy', 'PassiveIncome', 'StudentLoans', 
                        'Dividends', 'Portfolio', 'NetWorth']
        
        for name in hashtag_names:
            if not Hashtag.query.filter_by(name=name.lower()).first():
                hashtag = Hashtag(
                    name=name.lower(),
                    post_count=random.randint(10, 200),
                    posts_today=random.randint(1, 20),
                    posts_this_week=random.randint(10, 100)
                )
        ...
        for post in posts:
            ...
            # Create score record
            if not PostScore.query.filter_by(post_id=post.id).first():
            ...
            # Link random hashtags to posts
            if random.random() > 0.3:  # 70% of posts get hashtags
                ...
                for hashtag in selected_hashtags:
                    if not PostHashtag.query.filter_by(post_id=post.id, hashtag_id=hashtag.id).first():
                        link = PostHashtag(post_id=post.id, hashtag_id=hashtag.id)
                        db.session.add(link)
        ...
        print("   python jobs.py start_scheduler")
        print("   Or manually: python jobs.py run_all")
