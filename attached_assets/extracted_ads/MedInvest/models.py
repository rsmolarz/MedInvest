...
# ----------------------
# Report action history (admin notes + audit)
# ----------------------
...
class ReportAction(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    report_id = db.Column(db.Integer, db.ForeignKey('content_report.id'), nullable=False, index=True)
    admin_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True)
    action = db.Column(db.String(32), nullable=False, index=True)  # no_action | hide | lock | warn | reopen
    note = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)

    report = db.relationship('ContentReport', foreign_keys=[report_id])
    admin = db.relationship('User', foreign_keys=[admin_user_id])


# ----------------------
# Moderator playbooks by cohort (guidance + SOP)
# ----------------------


class CohortPlaybook(db.Model):
    __tablename__ = 'cohort_playbooks'

    id = db.Column(db.Integer, primary_key=True)
    cohort_dimension = db.Column(db.String(20), nullable=False, index=True)  # specialty, role, sponsor
    cohort_value = db.Column(db.String(120), nullable=False, index=True)
    title = db.Column(db.String(200), nullable=False)
    guidelines = db.Column(db.Text, nullable=False)  # markdown/text
    escalation_steps = db.Column(db.Text)  # markdown/text
    examples_allowed = db.Column(db.Text)
    examples_disallowed = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    __table_args__ = (
        db.UniqueConstraint('cohort_dimension', 'cohort_value', name='uq_cohort_playbook'),
    )
    ...
    def to_dict(self):
        return {
            ...
            'examples_allowed': self.examples_allowed,
            'examples_disallowed': self.examples_disallowed,
            'created_at': self.created_at.isoformat() + 'Z' if self.created_at else None,
