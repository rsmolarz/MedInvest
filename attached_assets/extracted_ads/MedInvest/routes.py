...
@app.route('/api/deals/<int:deal_id>/memory', methods=['GET'])
@login_required
@require_verified
def api_deal_memory(deal_id: int):
    """Return similar prior deal outcomes to create a learning loop."""
    DealDetails.query.get_or_404(deal_id)
    from deal_memory import find_similar_deal_outcomes
    items = find_similar_deal_outcomes(deal_id=deal_id, limit=5)
    return jsonify({'deal_id': deal_id, 'items': items}), 200
...
@login_required
@require_roles('admin')
def api_admin_reports_get(report_id: int):
    """Fetch a single report with history and a snapshot of content."""
    r = ContentReport.query.get_or_404(report_id)

    # History (admin notes)
    actions = ReportAction.query.filter_by(report_id=r.id).order_by(ReportAction.created_at.asc()).all()
    history = []
    for a in actions:
        admin_name = None
        try:
            admin_name = a.admin.full_name if a.admin else None
        except Exception:
            admin_name = None
        history.append({
            'id': a.id,
    ...
    return jsonify({'report': r.to_dict(), 'history': history, 'content': content}), 200
...
def api_admin_reports_resolve(report_id: int):
    ...
    if resolution not in ('no_action', 'hide', 'lock', 'warn'):
...
# ----------------------
# Moderator playbooks by cohort
# ----------------------


@app.route('/api/admin/moderation/playbooks', methods=['GET', 'POST'])
@login_required
@require_roles('admin')
def api_admin_playbooks():
    """CRUD for cohort-specific moderation playbooks (guidance + SOP)."""
    if request.method == 'GET':
        dim = (request.args.get('dimension') or '').strip().lower() or None
        val = (request.args.get('value') or '').strip() or None
        q = CohortPlaybook.query
        if dim:
            q = q.filter(CohortPlaybook.cohort_dimension == dim)
        if val:
            q = q.filter(CohortPlaybook.cohort_value == val)
        rows = q.order_by(CohortPlaybook.updated_at.desc()).limit(200).all()
        return jsonify({'playbooks': [p.to_dict() for p in rows]}), 200

    data = request.get_json(silent=True) or {}
    cohort_dimension = (data.get('cohort_dimension') or '').strip().lower()
    cohort_value = (data.get('cohort_value') or '').strip()
    title = (data.get('title') or '').strip()
    guidelines = (data.get('guidelines') or '').strip()
    escalation_steps = (data.get('escalation_steps') or '').strip() or None
    examples_allowed = (data.get('examples_allowed') or '').strip() or None
    examples_disallowed = (data.get('examples_disallowed') or '').strip() or None

    if not cohort_dimension or not cohort_value or not title or not guidelines:
        return jsonify({'error': 'missing_fields'}), 400

    # Upsert by cohort (one playbook per cohort dimension/value)
    p = CohortPlaybook.query.filter_by(cohort_dimension=cohort_dimension, cohort_value=cohort_value).first()
    if not p:
        p = CohortPlaybook(
            ...
            examples_allowed=examples_allowed,
            examples_disallowed=examples_disallowed,
        )
        db.session.add(p)
    else:
        p.title = title
        p.guidelines = guidelines
        p.escalation_steps = escalation_steps
        p.examples_allowed = examples_allowed
        p.examples_disallowed = examples_disallowed

    db.session.commit()
    return jsonify({'ok': True, 'playbook': p.to_dict()}), 200


@app.route('/api/admin/moderation/playbooks/<int:playbook_id>', methods=['DELETE'])
@login_required
@require_roles('admin')
def api_admin_playbooks_delete(playbook_id: int):
    p = CohortPlaybook.query.get_or_404(playbook_id)
    db.session.delete(p)
    db.session.commit()
    return jsonify({'ok': True}), 200
...
def api_admin_audit_export():
    """Export trust & moderation events as CSV or PDF.
    ...
    # Pull the key audit surfaces
    moderation = ModerationEvent.query.filter(ModerationEvent.created_at >= since).order_by(ModerationEvent.created_at.desc()).limit(5000).all()
    reports = ContentReport.query.filter(ContentReport.created_at >= since).order_by(ContentReport.created_at.desc()).limit(5000).all()
    actions = ReportAction.query.filter(ReportAction.created_at >= since).order_by(ReportAction.created_at.desc()).limit(5000).all()
    verifications = VerificationQueueEntry.query.filter(VerificationQueueEntry.created_at >= since).order_by(VerificationQueueEntry.created_at.desc()).limit(5000).all()
    invites = Invite.query.filter(Invite.created_at >= since).order_by(Invite.created_at.desc()).limit(5000).all()
    ...
    if fmt == 'pdf':
        ...
        def _line(txt: str, dy: int = 14):
            nonlocal y
            if y < 60:
                c.showPage()
                y = height - 50
            c.drawString(50, y, (txt or '')[:110])
            y -= dy

        _line(f"MedInvest Audit Export (last {days_i} days)", dy=18)
        _line(f"Generated: {datetime.utcnow().isoformat()}Z", dy=18)
        _line(" ")

        _line("Moderation events:")
        for m in moderation[:250]:
            _line(f"[{m.created_at.isoformat()}Z] {m.action} {m.entity_type}#{m.entity_id} user:{m.user_id} score:{m.signal_score:.2f} reason:{m.reason or ''}")

        _line(" ")
        _line("Reports:")
        for r in reports[:250]:
            _line(f"[{(r.created_at.isoformat()+'Z') if r.created_at else ''}] report#{r.id} {r.entity_type}#{r.entity_id} by:{r.reporter_user_id} status:{r.status} res:{r.resolution or ''}")

        _line(" ")
        _line("Report actions (admin notes):")
        for a in actions[:250]:
            _line(f"[{(a.created_at.isoformat()+'Z') if a.created_at else ''}] report#{a.report_id} admin:{a.admin_user_id} action:{a.action} note:{(a.note or '')[:60]}")

        _line(" ")
        _line("Verification queue:")
        for v in verifications[:250]:
            _line(f"[{(v.created_at.isoformat()+'Z') if v.created_at else ''}] user:{v.user_id} status:{v.status} assigned:{v.assigned_reviewer_id or ''}")

        _line(" ")
        _line("Invites:")
        for inv in invites[:250]:
            _line(f"[{(inv.created_at.isoformat()+'Z') if inv.created_at else ''}] code:{inv.code} issuer:{inv.issuer_user_id} accepted_by:{inv.accepted_by_user_id or ''}")

        c.save()
        buf.seek(0)
        from flask import send_file
        return send_file(buf, as_attachment=True, download_name=f"medinvest_audit_{days_i}d.pdf", mimetype='application/pdf')
    ...
    out = StringIO()
    w = csv.writer(out)
    w.writerow(["section", "ts", "entity", "details"])

    for m in moderation:
        w.writerow(["moderation", m.created_at.isoformat() + "Z", f"{m.entity_type}#{m.entity_id}", f"action={m.action}; user={m.user_id}; score={m.signal_score}; reason={m.reason or ''}"])
    for r in reports:
        ts = (r.created_at.isoformat() + "Z") if r.created_at else ""
        w.writerow(["report", ts, f"{r.entity_type}#{r.entity_id}", f"report_id={r.id}; reporter={r.reporter_user_id}; status={r.status}; resolution={r.resolution or ''}"])
    for a in actions:
        ts = (a.created_at.isoformat() + "Z") if a.created_at else ""
        note_clean = ((a.note or '').replace("\n", " "))[:500]
        w.writerow(["report_action", ts, f"report#{a.report_id}", f"admin={a.admin_user_id}; action={a.action}; note={note_clean}"])
    for v in verifications:
        ts = (v.created_at.isoformat() + "Z") if v.created_at else ""
        w.writerow(["verification", ts, f"user#{v.user_id}", f"status={v.status}; assigned={v.assigned_reviewer_id or ''}; npi={v.npi or ''}; role={(v.claimed_role or '')}"])
    for inv in invites:
        ts = (inv.created_at.isoformat() + "Z") if inv.created_at else ""
        w.writerow(["invite", ts, f"invite#{inv.id}", f"code={inv.code}; issuer={inv.issuer_user_id}; accepted_by={inv.accepted_by_user_id or ''}; source={(inv.source or '')}"])

    resp = Response(out.getvalue(), mimetype='text/csv')
    resp.headers['Content-Disposition'] = f"attachment; filename=medinvest_audit_{days_i}d.csv"
    return resp
