...
def can(user: Any, action: str, resource: Optional[Any] = None, **ctx: Any) -> Decision:
    ...
    if action in {Actions.VIEW_ANALYTICS, Actions.VIEW_REPORTS, Actions.MANAGE_SPONSORS}:
        # Admin-only for now.
        return Decision(False, "forbidden")

    if action in {Actions.MANAGE_INVITES}:
        # Verified users can create invites; non-verified denied by the gate above.
        return Decision(True, "ok")

    if action in {Actions.SUBMIT_REPORT}:
        return Decision(True, "ok")
