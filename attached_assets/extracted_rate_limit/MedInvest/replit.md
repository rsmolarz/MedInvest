...
This project uses Alembic for schema migrations. `app.py` no longer auto-creates
tables by default.
...
### Existing development DB created via `db.create_all()`

If you previously relied on automatic table creation, either:

1) delete the local sqlite file and re-run `alembic upgrade head`, **or**
2) create a one-time "baseline stamp" migration flow (more careful for production).
...
- `DATABASE_URL` (recommended): Postgres connection string or sqlite path
- `AUTO_CREATE_DB=1` (dev only): enables legacy `db.create_all()` on startup

## AI background jobs

The API supports async AI work via a DB-backed job queue (`ai_jobs`).

Run the web app and the worker as separate processes:
...
AI settings:
- `OPENAI_API_KEY` to enable OpenAI
