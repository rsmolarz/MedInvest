"""
Replit Auth Integration - Google, Apple, GitHub, and email login
"""
import jwt
import os
import uuid
...
def find_or_create_user(user_claims):
    """Find existing user by email or create new one from OAuth claims"""
    email = user_claims.get('email')
    replit_id = user_claims['sub']
    ...
    if user:
        ...
        if user_claims.get('profile_image_url') and not user.profile_image_url:
            user.profile_image_url = user_claims.get('profile_image_url')
        db.session.commit()
        return user
    
    first_name = user_claims.get('first_name') or 'New'
    last_name = user_claims.get('last_name') or 'User'
    
    user = User(
        email=email or f"user_{replit_id}@medinvest.local",
        first_name=first_name,
        last_name=last_name,
        replit_id=replit_id,
        profile_image_url=user_claims.get('profile_image_url'),
        specialty='',
        medical_license=f'OAUTH-{replit_id}',
    )
