...
def feed():
    ...
    try:
        ...
        return render_template(
            'error.html',
            error_code=500,
            error_message=f'Feed error: {str(e)[:200]}'), 500
...
def _feed_internal():
    ...
    if feed_type == 'latest':
        ...
        post_items = generate_feed(current_user,
                                   db,
                                   page=page,
                                   per_page=per_page)
    ...
    if current_user.is_authenticated and post_items:
        ...
        votes = PostVote.query.filter(
            ...
            PostVote.user_id == current_user.id).all()
    ...
    if not trending_hashtags:
        trending = [
            'BackdoorRoth', 'RealEstate', 'IndexFunds', 'PSLF', 'FIRE',
            'Retirement', 'TaxStrategy', 'PassiveIncome'
        ]
    else:
    ...
    try:
        feed_articles = get_medical_investment_news(
            limit=3) if page == 1 else []
    except:
        feed_articles = []

    # Get Bloomberg headlines for ticker
    try:
        bloomberg_headlines = get_bloomberg_headlines(
            limit=10) if page == 1 else []
    except:
        bloomberg_headlines = []
    ...
    # Get YouTube Shorts from The Medicine and Money Show
    youtube_shorts = []
    if page == 1:
        try:
            from utils.youtube_live import get_channel_shorts
            youtube_shorts = get_channel_shorts(max_results=6)
        except Exception as e:
            logging.error(f"Error loading YouTube shorts: {e}")
            youtube_shorts = []
    ...
    # Create mixed feed items (posts + news + MIA alerts + YouTube Shorts)
    mixed_feed = []
    ...
    shorts_positions = [3, 8, 15]  # Insert shorts after these post positions
    news_idx = 0
    mia_idx = 0
    shorts_idx = 0
    for i, post in enumerate(post_items):
        ...
        if i + 1 in mia_positions and mia_idx < len(mia_alerts):
            ...
            mia_idx += 1
        if i + 1 in shorts_positions and shorts_idx < len(youtube_shorts):
            mixed_feed.append({
                'type': 'youtube_short',
                'item': youtube_shorts[shorts_idx]
            })
            shorts_idx += 1
...
def create_post():
    ...
    for i, media in enumerate(media_files):
        post_media = PostMedia(post_id=post.id,
    ...
    current_user.add_points(
        5 if post_type == 'text' else 10)  # More points for media posts
        ...
        for username in mentioned_usernames:
            ...
            mentioned_user = User.query.filter(
                db.or_(
                    db.func.lower(
                        db.func.replace(
                            db.func.concat(User.first_name, User.last_name),
                            "'", "")) == username_clean,
                    db.func.lower(db.func.replace(
                        User.first_name, "'", "")) == username_clean)).first()
            if mentioned_user and mentioned_user.id != current_user.id:
                mention = PostMention(post_id=post.id,
...
def create_post_ajax():
    ...
    try:
            ...
            for username in mentioned_usernames:
                ...
                mentioned_user = User.query.filter(
                    db.or_(
                        db.func.lower(
                            db.func.replace(
                                db.func.concat(User.first_name,
                                               User.last_name), "'",
                                "")) == username_clean,
                        db.func.lower(
                            db.func.replace(User.first_name, "'",
                                            "")) == username_clean)).first()
                if mentioned_user and mentioned_user.id != current_user.id:
                    mention = PostMention(post_id=post.id,
                                          mentioned_user_id=mentioned_user.id)
                    db.session.add(mention)
        ...
        # Share to Facebook Page if configured (non-anonymous posts only)
        logging.info(
            f"AJAX Post created: id={post.id}, is_anonymous={is_anonymous}")
        if not is_anonymous and is_facebook_configured():
            author_name = f"{current_user.first_name} {current_user.last_name}".strip(
            ) or "A physician"
            logging.info(
                f"AJAX: Calling share_platform_post for post {post.id}")
            share_platform_post(post, author_name=author_name)
        else:
            logging.info(
                f"AJAX: Skipping Facebook share: is_anonymous={is_anonymous}")
...
def vote_post(post_id):
    ...
    existing_vote = PostVote.query.filter_by(post_id=post_id,
...
def profile(user_id=None):
    ...
    for skill in skills:
        user_endorsements[skill.id] = SkillEndorsement.query.filter_by(
            skill_id=skill.id, endorser_id=current_user.id).first() is not None

    recommendations = Recommendation.query.filter_by(
        user_id=user.id,
        is_visible=True).order_by(Recommendation.created_at.desc()).all()
...
def toggle_profile_privacy():
    ...
    current_user.is_profile_public = not getattr(current_user,
...
def add_skill():
    ...
    existing = InvestmentSkill.query.filter_by(user_id=current_user.id,
                                               name=name).first()
    ...
    skill = InvestmentSkill(user_id=current_user.id,
                            name=name,
                            category=category)
...
def endorse_skill(skill_id):
    ...
    existing = SkillEndorsement.query.filter_by(
        skill_id=skill_id, endorser_id=current_user.id).first()
    ...
    if existing:
        ...
        endorsement = SkillEndorsement(skill_id=skill_id,
                                       endorser_id=current_user.id)
        db.session.add(endorsement)
        db.session.commit()
        flash(f'You endorsed {skill.user.first_name}\'s {skill.name} skill!',
              'success')
...
def add_recommendation(user_id):
    ...
    existing = Recommendation.query.filter_by(
        user_id=user_id, author_id=current_user.id).first()
    ...
    if existing:
        ...
        flash(f'Your recommendation for {user.first_name} has been added!',
              'success')
...
def hashtag_page(tag_name):
    ...
    post_ids = [
        ph.post_id
        for ph in PostHashtag.query.filter_by(hashtag_id=hashtag.id).all()
    ]
...
def search():
    ...
    if search_type in ['all', 'users']:
        users = User.query.filter(
            db.or_(User.first_name.ilike(f'%{query}%'),
                   User.last_name.ilike(f'%{query}%'),
                   User.email.ilike(f'%{query}%'))).limit(20).all()
        results['users'] = users
...
def network():
    ...
    try:
        ...
        accepted_connections = Connection.query.filter(
            db.or_(Connection.requester_id == current_user.id,
        ...
        pending_connections = Connection.query.filter_by(
            addressee_id=current_user.id, status='pending').order_by(
                Connection.created_at.desc()).limit(50).all()
        pending_users = [
            User.query.get(c.requester_id) for c in pending_connections
        ]
        ...
        outgoing_connections = Connection.query.filter_by(
            requester_id=current_user.id, status='pending').order_by(
                Connection.created_at.desc()).limit(50).all()
        outgoing_users = [
            User.query.get(c.addressee_id) for c in outgoing_connections
        ]
        ...
        all_connections = Connection.query.filter(
            db.or_(Connection.requester_id == current_user.id,
                   Connection.addressee_id == current_user.id)).all()
        ...
        if current_user.license_state:
            near_me_query = User.query.filter(
                User.license_state == current_user.license_state, User.id
                != current_user.id)
            if exclude_ids:
                near_me_query = near_me_query.filter(
                    User.id.notin_(list(exclude_ids)))
            near_me = near_me_query.order_by(
                User.points.desc()).limit(20).all()

        # People in the same specialty
        same_specialty = []
        if current_user.specialty:
            same_specialty_query = User.query.filter(
                User.specialty == current_user.specialty, User.id
                != current_user.id)
            if exclude_ids:
                same_specialty_query = same_specialty_query.filter(
                    User.id.notin_(list(exclude_ids)))
            same_specialty = same_specialty_query.order_by(
                User.points.desc()).limit(20).all()

        # Colleagues = all accepted connections
        colleagues = []
        if all_connected_ids:
            colleagues = User.query.filter(User.id.in_(
                list(all_connected_ids))).order_by(User.last_name).all()
        ...
        try:
            ...
            new_members_query = User.query.filter(User.created_at >= week_ago,
                                                  User.id != current_user.id)
            if exclude_ids:
                new_members_query = new_members_query.filter(
                    User.id.notin_(list(exclude_ids)))
            new_members = new_members_query.order_by(
...
def update_settings():
    ...
    if setting_type == 'communication':
        current_user.email_notifications = request.form.get(
...
def disconnect_social(provider):
    ...
    if not has_password and connected_count <= 1:
        flash(
            'You must keep at least one sign-in method. Set a password first or connect another account.',
            'error')
        return redirect(url_for('main.settings', tab='account'))
...
def session_activity():
    ...
    sessions = LoginSession.query.filter_by(user_id=current_user.id).order_by(
        LoginSession.created_at.desc()).limit(50).all()
...
def authorized_apps():
    ...
    apps = AuthorizedApp.query.filter_by(
        user_id=current_user.id,
        is_active=True).order_by(AuthorizedApp.authorized_at.desc()).all()
...
def revoke_app(app_id):
    ...
    from models import AuthorizedApp
    app = AuthorizedApp.query.filter_by(
...
def api_search_users():
    ...
    users = User.query.filter(
        db.or_(
            ...
            User.last_name.ilike(f'{query}%'),
            db.func.concat(User.first_name,
                           User.last_name).ilike(f'{query}%'))).limit(8).all()

    results = []
    for u in users:
        # Create a unique mention handle: FirstnameLastname (no spaces)
        handle = f"{u.first_name}{u.last_name}".replace(' ',
                                                        '').replace("'", '')
        results.append({
            'id':
            u.id,
            'name':
            u.full_name.replace("'", "\\'"),  # Escape for JS
            'username':
            handle,
            'specialty': (u.specialty or '').replace('_', ' ').title(),
            'avatar':
            u.first_name[0] +
            u.last_name[0] if u.last_name else u.first_name[0],
            'is_verified':
            u.is_verified,
            'is_premium':
            u.is_premium
...
def report_bug():
    ...
    if request.method == 'POST':
        ...
        report = BugReport(reporter_id=current_user.id,
                           category=category,
                           title=title,
        ...
        flash(
            'Thank you! Your report has been submitted and our team will review it.',
            'success')
